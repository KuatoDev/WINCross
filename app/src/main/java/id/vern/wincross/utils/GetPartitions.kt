...
object GetPartitions {
    ...
    val slotValue =
        when {
          result.isSuccess &&
              ...
              result.out[0].contains("a", ignoreCase = true) -> "slot_a"
          result.isSuccess &&
              result.out.isNotEmpty() &&
              result.out[0].contains("b", ignoreCase = true) -> "slot_b"
          else -> "a_only"
        }
  ...
  private fun findPartition(partitionName: String): String? {
    val searchPaths = listOf("/dev/block/bootdevice/by-name/", "/dev/block/by-name/")
    ...
    for (path in searchPaths) {
      ...
      if (result.isSuccess && result.out.isNotEmpty()) {
        val filteredPaths = result.out.filter { it.endsWith("/$partitionName") }
  ...
  fun checkAndSaveAllPartitionsIfNeeded(context: Context) {
    ...
    val needsRefresh =
        partitionInfoList.any { info -> prefs.getString(info.key, null).isNullOrBlank() }
