...
object NotificationHelper {
  ...
  fun updateDownloadProgress(
    ...
    val notificationManager =
        context.getSystemService(Context.NOTIFICATION_SERVICE) as NotificationManager
    val updatedNotification = builder.setContentText(text).setProgress(100, progress, false).build()
    notificationManager.notify(notificationId, updatedNotification)
  }

  fun showCompletionNotification(context: Context, notificationId: Int, success: Boolean) {
    val notificationManager =
        context.getSystemService(Context.NOTIFICATION_SERVICE) as NotificationManager
    val notification =
        NotificationCompat.Builder(context, NOTIFICATION_CHANNEL_ID)
            ...
            .setContentText(
                if (success) "All files downloaded successfully"
                else "Failed to download some files")
            .setSmallIcon(
