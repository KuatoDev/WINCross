...
class UefiUpdateManager(private val activity: MainActivity) {
  ...
  fun checkForUpdates() {
    ...
    val deviceModel = prefs.getString("device_model", null) ?: Utils.getDeviceModel(activity)

    getDeviceUpdate(deviceModel)?.let { (owner, repo) -> startUpdateCheck(owner, repo) }
  }

  private fun shouldCheckUpdate(): Boolean {
    return prefs.getBoolean("uefi_auto_update", false) && !isShowingUpdateDialog
  }

  private fun getDeviceUpdate(deviceModel: String): DeviceUpdate? =
      when {
        deviceModel.contains("vayu", true) -> DeviceUpdate("woa-vayu", "POCOX3Pro-Releases")
        deviceModel.contains("nabu", true) -> DeviceUpdate("erdilS", "Port-Windows-11-Xiaomi-Pad-5")
        deviceModel.contains("a52s", true) -> DeviceUpdate("woa-a52s", "Samsung-A52s-5G-Releases")
        deviceModel.contains("beryllium", true) -> DeviceUpdate("n00b69", "woa-beryllium")
        deviceModel.contains("cepheus", true) -> DeviceUpdate("qaz6750", "XiaoMi9-Drivers")
        else -> null
      }
  ...
  private fun startUpdateCheck(owner: String, repo: String) {
    ...
    activity.lifecycleScope.launch {
      try {
        ...
        UtilityHelper.showToast(activity, "Failed to check for updates: ${e.message}")
      }
  ...
  private fun showUpdateDialog(latestVersion: String, downloadUrl: String) {
    ...
    DialogHelper.createDialog(
        ...
        message =
            activity.getString(
                R.string.update_dialog_message, UEFIHelper.getCurrentVersion(), latestVersion),
