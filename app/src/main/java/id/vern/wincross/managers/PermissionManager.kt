...
class PermissionManager(private val activity: MainActivity) {
    ...
    if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.S) {
      ...
      if (!alarmManager.canScheduleExactAlarms() &&
        ...
        try {
          ...
          activity.startActivity(intent)
          activity.sharedPreferences.edit().putBoolean(EXACT_ALARM_REQUESTED_KEY, true).apply()
        } catch (e: Exception) {
          Log.e("Permission", "Failed to open permission request: ${e.message}")
        }
      }
  ...
  private fun checkRequiredPermissions() {
    val permissions = listOf(Manifest.permission.RECEIVE_BOOT_COMPLETED)
  ...
  fun handlePermissionResult(
    ...
    if (requestCode == PERMISSION_REQUEST_CODE) {
      ...
      if (deniedPermissions.isNotEmpty()) {
        deniedPermissions.forEach { Log.d("Permission", "Permission denied: ${it.value}") }
      } else {
