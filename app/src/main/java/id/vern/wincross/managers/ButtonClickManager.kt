...
class ButtonClickManager(private val activity: MainActivity) {
        ...
        activity.checkRoot {
            DialogHelper.showBackupKernelPopup(activity, rootView) {
                lifecycleScope.launch(Dispatchers.IO) {
                    ...
                    prefs.edit()
                        .putLong(MainActivity.LAST_BACKUP_KEY, System.currentTimeMillis())
                        .apply()
    ...
    fun handleSwitchToWindows() {
        activity.checkRoot {
            DialogHelper.showFlashUefiPopup(activity, rootView) {
                FlashOperation.flashIt(activity)
            }
        }
    }
    ...
    fun handleSelectUEFI() {
        DialogHelper.showUEFIOptionsDialog(activity, rootView) {
            lifecycleScope.launch {
                ...
                activity.isUefiAvailable = if (selectedUefiPath.isNullOrEmpty()) {
