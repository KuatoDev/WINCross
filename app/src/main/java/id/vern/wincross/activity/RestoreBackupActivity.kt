...
class RestoreBackupActivity : AppCompatActivity() {
  ...
  override fun onCreate(savedInstanceState: Bundle?) {
    ...
    adapter =
        PartitionAdapter(partitions) { position, isChecked ->
    ...
    binding.btnRestore.setOnClickListener { showRestoreConfirmation() }
  ...
  private fun loadBackupPartitions() {
    ...
    val backupFiles = backupDir.listFiles { file -> file.name.endsWith(".img") }
  ...
  private fun selectAllPartitions(select: Boolean) {
    partitions.forEach { it.isSelected = select }
    adapter.notifyDataSetChanged()
    updateButtonStates()
  }

  private fun updateButtonStates() {
    val anySelected = partitions.any { it.isSelected }
    val allSelected = partitions.all { it.isSelected } && partitions.isNotEmpty()
  ...
  private fun showRestoreConfirmation() {
    val selectedPartitions = partitions.filter { it.isSelected }
    ...
    AlertDialog.Builder(this)
        ...
        .setPositiveButton("Restore") { _, _ -> startRestoreProcess(selectedPartitions) }
  ...
  inner class PartitionAdapter(
    ...
    inner class ViewHolder(private val binding: ItemPartitionBinding) :
      ...
      init {
        ...
        binding.checkboxPartition.setOnCheckedChangeListener { _, isChecked ->
          val position = adapterPosition
          if (position != RecyclerView.NO_POSITION) {
