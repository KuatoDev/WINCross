...
object ARMSoftware {
  ...
  private val urlFiles =
      listOf("WorksOnWoa.url", "TestedSoftware.url", "ARMSoftware.url", "ARMRepo.url")
  ...
  private suspend fun extractFiles(context: Context, destinationPath: String): ExtractionResult =
      withContext(Dispatchers.IO) {
        var successCount = 0
        var failureCount = 0

        urlFiles.forEach { fileName ->
          try {
  ...
  private suspend fun showResult(context: Context, result: ExtractionResult) =
        ...
        val (message, logMessage) =
            when (result) {
              ...
              ExtractionResult.FAILURE -> {
                context.getString(R.string.extract_failed_general) to "Extraction failed completely"
              }
            }
  ...
  private suspend fun showError(context: Context) =
      withContext(Dispatchers.Main) {
        DialogHelper.showPopupNotifications(
            context, context.getString(R.string.extract_failed_general))
      }
