...
object RotationToggle {
  private const val TAG = "RotationToggle"
  private const val PREFS_NAME = "WinCross_preferences"
  private const val PREF_MOUNT_TO_MNT = "Windows Mount Path"

  private data class ExtractPaths(val toolboxPath: String, val desktopPath: String)
  ...
  private val fileGroups =
      mapOf(
          "toolbox" to
              FileGroup(files = listOf("display.exe"), destinationPath = { it.toolboxPath }),
          "desktop" to
              FileGroup(
                  files = listOf("RotationShortcut.lnk", "RotationShortcutReverseLandscape.lnk"),
                  destinationPath = { it.desktopPath }))
  ...
  private fun getPaths(context: Context): ExtractPaths {
    ...
    return ExtractPaths(
        toolboxPath = "$basePath/Toolbox", desktopPath = "$basePath/Users/Public/Desktop")
  }
