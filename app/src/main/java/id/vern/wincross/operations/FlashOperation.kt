...
object FlashOperation {
  ...
  fun flashIt(context: Context) {
    ...
    coroutineScope.launch {
      ...
      // Step 2 & 3: Get user preferences and check UEFI
      val prefs = context.getSharedPreferences(PREFERENCES_NAME, Context.MODE_PRIVATE)
      var selectedUefiPath = prefs.getString("UEFI", null)

      // Check if UEFI file exists
      if (selectedUefiPath == null || !UtilityHelper.isFileExists(selectedUefiPath)) {
        ...
        val dialogResult =
            suspendCancellableCoroutine<Boolean> { continuation ->
              ...
              DialogHelper.createDialog(
                  ...
                  onNegative = { continuation.resume(false) })
            }
        ...
        // Recheck UEFI file after dialog
      ...
      // Step 4: Check if logo flashing is enabled and file exists
      val flashLogoWithUefi = prefs.getBoolean("flash_logo_with_uefi", false)
      val winlogoImagePath = "$logoFolderPath/winlogo.img"

      if (flashLogoWithUefi && !UtilityHelper.isFileExists(winlogoImagePath)) {
        UtilityHelper.showToast(
            context,
            context.getString(
                R.string.flash_status,
      ...
      if (backupIfEmpty || forceBackup) {
        ...
        if (forceBackup || !fileExists) {
          val backupSuccess =
              withContext(Dispatchers.IO) { BackupOperation.backupBootToWindows(context) }
